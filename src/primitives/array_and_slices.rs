...
//created using brackets [], and their length, which is known at compile time, is part of their
...
fn main() {
    ...
    println!("Values are initialized to {}", ys[1]);

    // Arrays are stack allocated
    println!("Array occupies {} bytes", mem::size_of_val(&xs)); // 20 bytes (5 positions x 4 bytes)

    // Slices can point to a section of an array
    // They are of the form [starting_index..ending_index]
    // starting_index is the first position in the slice
    // ending_index is once more than the last position in the slice
